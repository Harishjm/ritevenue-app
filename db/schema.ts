...
export const venuePhotos=sqliteTable('venue_photo',{id:text('id').primaryKey(),venueId:text('venue_id').notNull(),storedFilename:text('stored_filename').notNull(),altText:text('alt_text').notNull(),displayOrder:integer('display_order').notNull()},t=>[uniqueIndex('idx_venue_photo_order').on(t.venueId,t.displayOrder),uniqueIndex('idx_venue_photo_filename').on(t.venueId,t.storedFilename)]);

...
export const publicVenueIntakes=sqliteTable('public_venue_intakes',{id:text('id').primaryKey(),requestKey:text('request_key').notNull().unique(),payloadHash:text('payload_hash').notNull(),dataJson:text('data_json').notNull(),status:text('status').notNull().default('new'),createdAt:text('created_at').notNull()},t=>[index('idx_public_intake_created').on(t.createdAt)]);

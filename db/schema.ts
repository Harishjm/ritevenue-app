...
export const demoSlots=sqliteTable('demo_slots',{venueSlug:text('venue_slug').notNull(),eventDate:text('event_date').notNull(),status:text('status').notNull(),holdId:text('hold_id').unique(),userId:text('user_id'),expiresAt:integer('expires_at'),quoteJson:text('quote_json'),bookingId:text('booking_id')},t=>[uniqueIndex('idx_demo_slot_venue_date').on(t.venueSlug,t.eventDate),index('idx_demo_slot_date').on(t.eventDate)]);

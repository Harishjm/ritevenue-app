...
export const draftSchema=z.object({id:z.string().uuid(),name:z.string().trim().min(3).max(100),locality:z.string().trim().min(2).max(100),address:z.string().trim().min(8).max(400),type:z.string().refine(v=>venueTypes.slice(1).includes(v)),capacity:z.number().int().min(1).max(2000),description:z.string().trim().min(40).max(3000),pricing:pricingSchema,images:z.array(z.string().uuid()).max(6),cateringPolicy:policySchema.default({}),publication:publicationSchema.default({}),rightsConfirmed:z.literal(true),submit:z.boolean()}).strict();

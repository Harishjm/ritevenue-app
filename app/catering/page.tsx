...
async function Protected({search}:{search:{venue?:string;date?:string;guests?:string}}){const user=await requireAuthenticatedUser('/catering');if(!isAdmin(user))return <main className="content-page"><h1>Catering is not part of this launch.</h1><a href="/" className="primary">Explore venues</a></main>;try{const catalog=await cateringCatalog();const date=dateSchema.safeParse(search.date);return <main className="content-page"><CateringExplorer initialCatalog={catalog} initialVenue={catalog.venues.some(v=>v.slug===search.venue)?search.venue!:''} initialDate={date.success?date.data:indiaToday()} initialGuests={Math.max(1,Math.min(2000,Math.floor(Number(search.guests)||100)))} today={indiaToday()}/></main>;}catch{return <main className="content-page"><h1>Catering is temporarily unavailable</h1><p>Please refresh to try again.</p><a href="/" className="primary">Back to venues</a></main>;}}

...
import {venueImageAlt,venueImageFilename} from '@/lib/venue-image';
...
export async function POST(request:Request,{params}:{params:Promise<{action:string}>}){
 try{const user=await apiUser(request);const {action:requestedAction}=await params;if(isPublicDirectory()&&['hold','confirm','release','start-checkout','complete-checkout','cancel-checkout'].includes(requestedAction))return json({error:'Online booking, date holds and payments are not available during the directory launch.'},410);const formFlow=['start-checkout','complete-checkout','cancel-checkout'].includes(requestedAction);const action=({'start-checkout':'hold','complete-checkout':'confirm','cancel-checkout':'release'} as Record<string,string>)[requestedAction]||requestedAction;const database=db();const now=Math.floor(Date.now()/1000);
  ...
  const type=request.headers.get('content-type')||'';if(type!=='image/webp')return reply({error:'Upload an image processed by the venue photo uploader (WebP).'},400);
  ...
  const valid=size>12&&new TextDecoder().decode(bytes.slice(0,4))==='RIFF'&&new TextDecoder().decode(bytes.slice(8,12))==='WEBP';if(!valid)return reply({error:'The file content does not match its image type.'},400);
    const id=crypto.randomUUID(),requestedPosition=Math.max(1,Number(request.headers.get('x-photo-position'))||1),name=request.headers.get('x-venue-name')||'venue',locality=request.headers.get('x-venue-locality')||'bengaluru',city=request.headers.get('x-venue-city')||'Bengaluru',venueId=request.headers.get('x-venue-id')||'unassigned',existing=venueId==='unassigned'?null:await database.prepare('SELECT COALESCE(MAX(display_order),0)+1 AS next FROM venue_photo WHERE venue_id=?').bind(venueId).first<{next:number}>(),position=existing?.next||requestedPosition,filename=request.headers.get('x-photo-filename')||venueImageFilename(name,locality,city,position),altText=request.headers.get('x-photo-alt')||venueImageAlt(name,locality,city,position),key=`owner-images/${id}/${filename}`;await bucket().put(key,bytes,{httpMetadata:{contentType:'image/webp'}});try{await database.batch([database.prepare('INSERT INTO owner_images (id,owner_id,object_key,content_type,created_at) VALUES (?,?,?,?,?)').bind(id,user.userId,key,'image/webp',new Date().toISOString()),database.prepare('INSERT INTO venue_photo (id,venue_id,stored_filename,alt_text,display_order) VALUES (?,?,?,?,?)').bind(id,venueId,filename,altText,position)]);}catch(e){await bucket().delete(key);throw e;}return reply({id},201);
  }
  ...
  if(action==='review_intake'){
   ...
   if(action==='convert_intake'){
    ...
    ...photos.map((photo,position)=>database.prepare("INSERT INTO venue_photo (id,venue_id,stored_filename,alt_text,display_order) SELECT ?,?,?,?,? WHERE EXISTS (SELECT 1 FROM owner_drafts WHERE id=?)").bind(photo.id,draftId,photo.object_key.split('/').pop()||photo.id,venueImageAlt(data.data.venueName,data.data.locality,'Bengaluru',position+1),position+1,draftId)),

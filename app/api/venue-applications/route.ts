...
export async function GET(){try{const user=await apiUser();if(!isAdmin(user))throw new Error('FORBIDDEN');const rows=await db().prepare('SELECT id,data_json,status,created_at FROM public_venue_intakes ORDER BY created_at DESC LIMIT 100').all<{id:string;data_json:string;status:string;created_at:string}>();return json({applications:rows.results.map(r=>({id:r.id,data:JSON.parse(r.data_json),status:r.status,createdAt:r.created_at}))});}catch(e){return apiError(e);}}

...
const bytes=new Uint8Array([255,216,255,224,0,0,0,0,0,0,0,0,0]);const upload=await api.POST(new Request(origin+'/api/demo/images',{method:'POST',headers:{Origin:origin,'Content-Type':'image/jpeg'},body:bytes}),{params:Promise.resolve({action:'images'})});assert.equal(upload.status,201);const image=await upload.json();assert.equal((await post('drafts',{...draft,images:[image.id],submit:true})).status,400,'A single photo cannot be submitted');
const upload2=await api.POST(new Request(origin+'/api/demo/images',{method:'POST',headers:{Origin:origin,'Content-Type':'image/jpeg'},body:bytes}),{params:Promise.resolve({action:'images'})});assert.equal(upload2.status,201);const image2=await upload2.json();
assert.notEqual(sql.prepare('SELECT venue_id FROM venue_photo WHERE id=?').get(image.id).venue_id,sql.prepare('SELECT venue_id FROM venue_photo WHERE id=?').get(image2.id).venue_id,'Uploads without a venue ID must not share a placeholder venue');
assert.equal((await post('drafts',{...draft,images:[image.id,image2.id],submit:true})).status,200);
assert.equal((await post('drafts',{...draft,images:Array(16).fill(image.id),submit:false})).status,400,'More than 15 photos are rejected');
assert.equal((await post('drafts',{...draft,images:[image.id,image.id],submit:false})).status,400,'Duplicate photos are rejected');
const additionalPhotos=await Promise.all(Array.from({length:13},async()=>{const response=await api.POST(new Request(origin+'/api/demo/images',{method:'POST',headers:{Origin:origin,'Content-Type':'image/jpeg'},body:bytes}),{params:Promise.resolve({action:'images'})});assert.equal(response.status,201);return (await response.json()).id;}));
assert.equal((await post('drafts',{...draft,id:crypto.randomUUID(),images:[image.id,image2.id,...additionalPhotos],submit:true})).status,200,'Fifteen distinct photos can be submitted');
const rejectedId=crypto.randomUUID(),rejectedDraft={...draft,id:rejectedId,name:'Incomplete venue submission',images:[image.id,image2.id],submit:true};assert.equal((await post('drafts',rejectedDraft)).status,200);
...
const amended={...draft,images:[image.id,image2.id],name:'Updated owner venue',pricing:{...updated,rent:1234500},submit:true};
...
const privatePackages=JSON.parse(sql.prepare('SELECT data_json FROM owner_drafts WHERE id=?').get(id).data_json);
privatePackages.packageAvailability={rent:'available',marriageRent:'not_applicable',morningRent:'available',eveningRent:'not_available'};
...
const directUpload2=await api.POST(new Request(origin+'/api/demo/images',{method:'POST',headers:{Origin:origin,'Content-Type':'image/jpeg'},body:bytes}),{params:Promise.resolve({action:'images'})});assert.equal(directUpload2.status,201);
const directPhoto=(await directUpload.json()).id,directPhoto2=(await directUpload2.json()).id,directId=crypto.randomUUID();
const directDraft={...photoDraft,id:directId,images:[directPhoto,directPhoto2],rightsConfirmed:true,submit:true,publication:adminRights};
...
const publicDirect=(await (await pget('catalog')).json()).venues.find(v=>v.slug==='owner-'+directId);assert.equal(publicDirect.authorizationSource,'admin');assert.equal(publicDirect.calendar,null);assert.equal(publicDirect.pricing,null);assert.equal(publicDirect.packageAvailability,null);assert.equal(publicDirect.cateringPolicy,null);
...
assert.equal((await post('drafts',{...photoDraft,images:[photoRecord.id,directPhoto],rightsConfirmed:true,submit:true,publication:{consent:true,calendar:null,calendarUpdatedAt:null}})).status,200);
...
const limitedPackages={rent:'not_applicable',marriageRent:'not_available',morningRent:'available',eveningRent:'not_applicable'};
const limitedDraft={...photoDraft,images:[photoRecord.id,directPhoto],packageAvailability:limitedPackages,rightsConfirmed:true,submit:true,publication:{consent:true,calendar:null,calendarUpdatedAt:null}};

...
const upload2=await api.POST(new Request(origin+'/api/demo/images',{method:'POST',headers:{Origin:origin,'Content-Type':'image/jpeg'},body:bytes}),{params:Promise.resolve({action:'images'})});assert.equal(upload2.status,201);const image2=await upload2.json();assert.equal((await post('drafts',{...draft,images:[image.id,image2.id],submit:true})).status,200);
